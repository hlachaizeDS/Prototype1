...
def multi_dispense_in_wells(hardware, lines_volumes_wells_dict, is384=0, max_vol=None):
    ...
    if isinstance(hardware, ProtoHardware):
        dispense_on_proto(hardware, lines_volumes_wells_dict, is384, max_vol)
    elif isinstance(hardware, MGATestbenchHardware):
        if not is384:
            raise ValueError(
                "MGATestbenchHardware can only dispense in 384 well plates"
            )
        hardware.dispense(lines_volumes_wells_dict)

    hardware.goToWell("thermalCamera", 1, 0)


def dispense_on_proto(hardware, lines_volumes_wells_dict, is384=0, max_vol=None):
    ...
    fake_plate_dims = [
        real_plate_dims[0] + 2 * (dispense_head_row_nb - 1),
        real_plate_dims[1] + 2 * (dispense_head_col_nb - 1),
    ]

    for col in range(
        0 - dispense_head_min_col, fake_plate_dims[1] - (dispense_head_max_col) + 1, 1
    ):
        for first_well in range(
            1 - dispense_head_min_row + fake_plate_dims[0] * col,
            1 + fake_plate_dims[0] * (col + 1) - dispense_head_max_row,
            1,
        ):
            dispense_dict = {}
            for line in lines_volumes_wells_copy.keys():
                if is384:
                    well = (
                        first_well
                        + 2 * dispense_head[line][0]
                        + fake_plate_dims[0] * 2 * dispense_head[line][1]
                    )
                else:
                    well = (
                        first_well
                        + dispense_head[line][0]
                        + fake_plate_dims[0] * dispense_head[line][1]
                    )

                real_well = fake_plate_well_to_real_well(
                    well, real_plate_dims, fake_plate_dims, dispHead_dims
                )
                ...
                if ((real_well - 1) % division_factor in accepted_modulos) and any(
                    well_to_dispense in lines_volumes_wells_copy[line][1]
                    for well_to_dispense in real_wells_all_nozzles
                ):
                    ...
                    if isinstance(lines_volumes_wells_copy[line][0], list):
                        ...
                        dispense_dict[line] = (
                            lines_volumes_wells_copy[line][0][column_real_well - 1]
                            * nozzle_nb
                        )
                    else:
                        dispense_dict[line] = (
                            lines_volumes_wells_copy[line][0] * nozzle_nb
                        )
...
def getParameters(synthesis_sheet):
    ...
    for row in range(param_indexes[0] + 1, synthesis_sheet.nrows):
        code = (
            synthesis_sheet.cell_value(row, 1)
            + "="
