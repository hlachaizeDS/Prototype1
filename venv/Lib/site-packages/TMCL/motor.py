...
class Motor(object):
    ...
    def stop_wait(self, hardware):
        self.send(Command.MST, 0, self.motor_id, 0)
        sleep(0.2)
        while self.axis.get(3) != 0:
    ...
    def move_absolute_wait(self, hardware, position):
        reply = self.send(Command.MVP, 0, self.motor_id, position)
        sleep(0.05)
        while self.axis.get(8) == 0:
            hardware.parent.update()
            sleep(0.05)

        return reply.status

    def move_relative_wait(self, hardware,offset):
